...
void ListModel::moveRowBack(int orig, const QModelIndex &parent) {
    beginMoveRows(parent, orig, orig, parent, orig - 1);
    m_list.move(orig, orig - 1);
    endMoveRows();
}

void ListModel::moveRowForward(int orig, const QModelIndex &parent) {
    beginMoveRows(parent, orig, orig, parent, orig + 2);
    m_list.move(orig, orig + 1);
    endMoveRows();
}
...
bool ListModel::removeRowsNoDeleteItems(int row, int count,
...
ListItem *ListModel::readRow(int row) { return m_list.at(row); }
