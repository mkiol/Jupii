...
struct Query {
    ...
    Query(const QString &query, QDBusUnixFileDescriptor &qfd)
        : m_query{query}, m_qfd{qfd} {}
    ...
    auto operator()(std::monostate) { return QDBusPendingReply<QStringList>{}; }
};
...
QString Tracker::genTrackerId(const QString &name) {
    ...
    return QString{
        QCryptographicHash::hash(striped.isEmpty() ? " " : striped.toUtf8(),
...
QString Tracker::genAlbumArtFile(const QString &albumName,
    ...
    const auto filepath = QString{"%1/media-art/album-%2-%3.jpeg"}.arg(
...
bool Tracker::pingTracker1() {
    ...
    if (resp.isError()) {
        qWarning() << "Tracker1 ping error:" << resp.error().name()
                   << resp.error().message();
        return false;
    }
...
bool Tracker::pingTracker3() {
    OrgFreedesktopDBusPeerInterface dbus_inf{
        "org.freedesktop.Tracker3.Miner.Files",
        "/org/freedesktop/Tracker3/Endpoint", QDBusConnection::sessionBus()};

    auto resp = dbus_inf.Ping();
    resp.waitForFinished();
    if (resp.isError()) {
        qWarning() << "Tracker3 ping error:" << resp.error().name()
                   << resp.error().message();
        return false;
    }
